...
def read_one_batch_pos_neg(data_root_folder, f1_index, f1_seq, train_imgf1, train_imgf2, train_dir1, train_dir2,
                           train_overlap, overlap_thresh):  # without end
...
if __name__ == '__main__':
    ...
    reference_frames, reference_gts, pos_num, neg_num = read_one_batch_pos_neg(seqs_root, cur_frame_idx, seq,
                                                                               train_imgf1, train_imgf2, train_dir1,
    ...
    plt.figure(figsize=(15, 3))
    plt.suptitle(
        "One sampled query-reference from KITTI sequence " + seq + ", Overlap: " + str(reference_gts[vis_idx].item()))
    plt.subplot(211)
    plt.title("query")
    plt.imshow(current_frame.cpu().detach().numpy()[0, 0, :, :])
    plt.subplot(212)
